    ...
    return (
        <div className='mt-4'>
            ...
            <div className='grid grid-cols-12 gap-6 md:grid-cols-12 sm:grid-cols-1'>
                {data.map((item) => {
                    return (
                        <div key={item._id} className='col-span-12 w-10/12 mx-auto p-8 container md:col-span-6 m-10 gap-40 shadow-md overflow-hidden hover:shadow-xl hover:scale-105 transform transition-all duration-500 ease-in-out lg:col-span-4  border-gray-950 rounded-2xl h-full '>
                            <figure className='w-full h-3/4 flex justify-center items-center'>
                                <Image className='w-full h-full object-fit rounded ' src={item.img} width={214} height={108} alt={item.title}></Image>
                            </figure>
